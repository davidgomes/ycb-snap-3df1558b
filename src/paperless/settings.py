...
# Cachalot: optional database read cache. The library is only loaded when enabled.
def _parse_read_cache_ttl() -> int:
    """
    Return the database read-cache TTL in seconds.

    Missing, non-numeric, and non-positive values fall back to one hour.
    Values above one year are capped at 31536000 seconds.
    """
    default_ttl = 3600
    max_ttl = 31536000  # 1 year
    raw_ttl = os.getenv("PAPERLESS_READ_CACHE_TTL")
    if raw_ttl is None:
        return default_ttl
    try:
        ttl = int(raw_ttl)
...
def _parse_cachalot_settings() -> dict[str, str | bool | int]:
...
def _parse_caches() -> dict[str, dict[str, str]]:
    return {
        "default": {
        ...
        "read-cache": {
            "BACKEND": _CACHE_BACKEND,
            "LOCATION": str(cachalot_settings["CACHALOT_REDIS_URL"]),
            "KEY_PREFIX": _REDIS_KEY_PREFIX,
        },
    }
