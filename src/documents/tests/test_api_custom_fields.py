...
class TestCustomFieldsAPI(DirectoriesMixin, APITestCase):
    ...
    def test_custom_field_select_update_old_version_preserves_ids(self):
        """
        GIVEN:
            - Existing select custom field with options that have ids
        WHEN:
            - API patch request is made with api version header < 7 using labels
        THEN:
            - Ids of options with unchanged labels are kept
            - New options get a new id
        """
        custom_field_select = CustomField.objects.create(
            name="Select Field",
            data_type=CustomField.FieldDataType.SELECT,
        ...
        resp = self.client.patch(
            ...
            data=json.dumps(
                {
                    "extra_data": {
                        "select_options": ["Option 2", "Option 3"],
                    },
                },
            ),
            content_type="application/json",
        ...
        options = custom_field_select.extra_data["select_options"]
        self.assertEqual(options[0], {"label": "Option 2", "id": "def-456"})
        self.assertEqual(options[1]["label"], "Option 3")
        self.assertNotIn(options[1]["id"], [None, "abc-123", "def-456"])

    def test_custom_field_select_value_old_version_invalid_index(self):
        """
        GIVEN:
            - Existing select custom field
        WHEN:
            - API patch request is made for a document with api version header < 7
