...
func (ge *gitlabExporter) exportBug(ctx context.Context, b *cache.BugCache, since time.Time, out chan<- core.ExportResult) {
	...
	if ok {
		if importedBaseURL, hasBase := snapshot.GetCreateMetadata(metaKeyGitlabBaseUrl); hasBase && importedBaseURL != "" {
			sameInstance, err := sameGitlabInstance(importedBaseURL, ge.conf[keyGitlabBaseUrl])
			if err != nil {
