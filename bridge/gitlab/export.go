...
func (ge *gitlabExporter) exportBug(ctx context.Context, b *cache.BugCache, since time.Time, out chan<- core.ExportResult) {
	...
	if ok {
		if gitlabBaseUrl, hasBase := snapshot.GetCreateMetadata(metaKeyGitlabBaseUrl); hasBase &&
			normalizeBaseURL(gitlabBaseUrl) != normalizeBaseURL(ge.conf[keyGitlabBaseUrl]) {
		...
		_, err = b.SetMetadata(
			...
			map[string]string{
				...
				metaKeyGitlabBaseUrl: normalizeBaseURL(ge.conf[keyGitlabBaseUrl]),
			},
		)
		if err != nil {
