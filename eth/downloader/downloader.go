...
func (d *Downloader) commitSnapSyncData(results []*fetchResult, stateSync *stateSync) error {
	...
	for i, result := range results {
		...
		// Pre-Canyon deposit nonces are not part of the receipt hash, so snap-sync
		// peers can supply the wrong nonce. Correct those from the embedded reference.
		if cfg := d.blockchain.Config(); cfg.IsOptimism() && !cfg.IsCanyon(blocks[i].Time()) {
			recs = correctReceipts(recs, result.Transactions, blocks[i].NumberU64(), cfg.ChainID)
		}
		receipts[i] = recs
	}
