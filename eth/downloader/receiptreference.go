...
func correctReceipts(receiptsRLP rlp.RawValue, transactions types.Transactions, blockNumber uint64, chainIDBig *big.Int) rlp.RawValue {
	...
	// Decode storage-format receipts. Snap sync delivers receipts without the
	// bloom filter, so the consensus receiptRLP shape (which includes Bloom)
	// cannot be used here.
	...
	for i, r := range receipts {
		if i >= len(transactions) {
			log.Warn("Receipt Correction: Receipt and transaction count mismatch", "receipts", len(receipts), "transactions", len(transactions))
			break
		}
		tx := transactions[i]
		...
		if from := tx.From(); from != systemAddress {
			...
			if r.DepositNonce == nil || nonce != *r.DepositNonce {
				...
				if r.DepositNonce != nil {
					previous = *r.DepositNonce
				}
				log.Warn("Receipt Correction: Corrected deposit nonce", "from", from, "nonce", previous, "corrected", nonce)
