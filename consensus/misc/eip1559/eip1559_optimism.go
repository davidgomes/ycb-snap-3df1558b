...
// ValidateOptimismExtraData validates the Optimism extraData of a block with the given timestamp.
...
// DecodeOptimismExtraData decodes the Optimism extraData of a block with the given timestamp.
// The extraData is expected to be valid (i.e. ValidateOptimismExtraData has been called previously).
...
// EncodeOptimismExtraData encodes the Optimism extraData of a block with the given timestamp.
// Will panic if Jovian is active and minBaseFee is nil.
...
// DecodeJovianExtraData decodes the Jovian extraData parameters from the encoded form defined here:
// https://specs.optimism.io/protocol/jovian/exec-engine.html
//
// Holocene-format (9-byte) extraData is decoded on a best-effort basis with a nil minBaseFee, so that
// the parent of the first Jovian block can be decoded. Returns 0,0,nil if the format is invalid, though
// ValidateJovianExtraData should be used instead of this function for validity checking.
...
// ValidateJovianExtraData checks if the header extraData is valid according to the Jovian upgrade.
func ValidateJovianExtraData(extra []byte) error {
	...
	if extra[0] != JovianExtraDataVersionByte {
		return fmt.Errorf("jovian extraData should have %d version byte, got %d", JovianExtraDataVersionByte, extra[0])
	}
