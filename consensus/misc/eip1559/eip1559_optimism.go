...
// ForkChecker reports which Optimism forks are active at a given timestamp.
// *params.ChainConfig implements this interface.
...
// It uses the config and block time to determine how to do the validation.
...
// It uses the config and block time to determine how to do the encoding.
...
func DecodeJovianExtraData(extra []byte) (uint64, uint64, *uint64) {
	...
	if len(extra) == 9 {
		// This is Holocene extraData (no minimum base fee).
...
// The result is 17 bytes: version byte (1) + 8 bytes of EIP-1559 parameters + 8 bytes of minimum base fee.
...
// Holocene's 9-byte encoding is rejected once Jovian is active.
