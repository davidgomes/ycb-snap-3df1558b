...
func CalcBaseFee(config *params.ChainConfig, parent *types.Header, time uint64) *big.Int {
	...
	// (post-Jovian) the minimum base fee from the parent header extraData.
	...
	// OP-Stack Jovian: clamp to the configured minimum. A nil minimum (pre-Jovian)
	// leaves the computed fee unchanged, including when it is very small.
	if minBaseFee != nil {
