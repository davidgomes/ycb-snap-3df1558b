...
func CalcBaseFee(config *params.ChainConfig, parent *types.Header, time uint64) *big.Int {
	...
	// OP-Stack: from Holocene onwards, decode denominator, elasticity and
	// minBaseFee from the parent extraData. Jovian adds the minimum base fee.
	...
	// If the computed base fee falls below the configured floor, use the floor.
	// A nil minBaseFee (pre-Jovian) leaves the computed value unchanged, even if it is very small.
	if minBaseFee != nil {
