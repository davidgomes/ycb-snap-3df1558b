...
func (beacon *Beacon) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header) error {
	...
	// Optimism Holocene/Jovian blocks must carry the fork's extraData encoding.
	// Pre-Holocene blocks, including genesis, keep their existing extraData rules.
	if chain.Config().IsOptimism() && chain.Config().IsHolocene(header.Time) {
