...
type generateParams struct {
	...
	minBaseFee    *uint64            // Optional minimum base fee (required post-Jovian)
...
func (miner *Miner) prepareWork(genParams *generateParams, witness bool) (*environment, error) {
	...
	if miner.chainConfig.IsHolocene(header.Time) {
		...
		if miner.chainConfig.IsJovian(header.Time) {
			if genParams.minBaseFee == nil {
				return nil, errors.New("expected minBaseFee post-Jovian, got none")
			}
	...
	if genParams.minBaseFee != nil && !miner.chainConfig.IsJovian(header.Time) {
		return nil, errors.New("got minBaseFee, expected none pre-Jovian")
	}
