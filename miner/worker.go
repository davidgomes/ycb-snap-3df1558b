...
func (miner *Miner) prepareWork(genParams *generateParams, witness bool) (*environment, error) {
	...
	// Jovian stores the DA footprint in BlobGasUsed. Ensure the field exists even if
	// the block is not on the Cancun blob schedule.
	if miner.chainConfig.IsDAFootprintBlockLimit(header.Time) && header.BlobGasUsed == nil {
		header.BlobGasUsed = new(uint64)
	}
