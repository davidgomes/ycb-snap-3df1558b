...
func (miner *Miner) generateWork(genParam *generateParams, witness bool) *newPayloadResult {
	...
	for _, tx := range genParam.txs {
		...
		// Force-included txs are not limited by the DA footprint, but non-deposit ones
		// still count towards it, matching the state processor.
...
func (miner *Miner) commitTransactions(env *environment, plainTxs, blobTxs *transactionsByPriceAndNonce, interrupt *atomic.Int32) error {
	...
	daFootprintLeft := new(big.Int).Sub(new(big.Int).SetUint64(gasLimit), new(big.Int).SetUint64(env.daFootprint))
