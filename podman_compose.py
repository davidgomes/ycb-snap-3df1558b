...
def try_parse_bool(value: Any) -> bool | None:
    """Parse bools from compose YAML or PODMAN_COMPOSE_* environment strings."""
...
class PodmanCompose:
    ...
    def resolve_pod_args(self) -> list[str]:
        ...
        # - Environment PODMAN_COMPOSE_POD_ARGS (already merged into x_podman)
    ...
    def _coerce_x_podman_env_value(self, key: PodmanCompose.XPodmanSettingKey, value: Any) -> Any:
        """Environment values arrive as strings. Match compose-file types."""
        if key == PodmanCompose.XPodmanSettingKey.POD_ARGS and isinstance(value, str):
            try:
        ...
        if key in (
            ...
            parsed_bool = try_parse_bool(value)
            if parsed_bool is not None:
                return parsed_bool
        return value

    def _parse_x_podman_settings(self, compose: dict[str, Any], environ: dict[str, str]) -> None:
        """Load global x-podman settings.

        Precedence (later wins): compose file, then PODMAN_COMPOSE_* environment
        variables. Command-line flags are applied later in resolve_in_pod and
        resolve_pod_args.
        """
        known_keys = {setting.value: setting for setting in PodmanCompose.XPodmanSettingKey}
        self.x_podman = {}

        for key, value in compose.get("x-podman", {}).items():
        ...
        for key, value in env.items():
            known_key = known_keys.get(key)
            if known_key:
                self.x_podman[known_key] = self._coerce_x_podman_env_value(known_key, value)
            else:
                log.warning(
