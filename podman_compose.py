...
class PodmanCompose:
    ...
    def resolve_pod_args(self) -> list[str]:
        ...
        # - PODMAN_COMPOSE_POD_ARGS / docker-compose.yml x-podman.pod_args
        # - Default value
    ...
    def _coerce_x_podman_env_value(
        self, key: PodmanCompose.XPodmanSettingKey, value: str
    ) -> Any:
        """Turn a PODMAN_COMPOSE_* string into the type the rest of the code expects."""
        ...
        if key in (
            ...
            if lowered in ("0", "false", "no", "off", ""):
                return False
            log.warning(
                "invalid boolean for %s=%s; expected true/false or 1/0",
                key.value,
                value,
            )
        # in_pod stays a string so custom pod names and "0"/"false" keep working
        return value

    def _parse_x_podman_settings(self, compose: dict[str, Any], environ: dict[str, str]) -> None:
        """Load x-podman settings from the compose file, then let PODMAN_COMPOSE_* override them.

        Command-line flags still win later in resolve_in_pod() and resolve_pod_args().
        """
        known_keys = {setting.value: setting for setting in PodmanCompose.XPodmanSettingKey}
        self.x_podman = {}

        file_settings = compose.get("x-podman") or {}
        if not isinstance(file_settings, dict):
            log.warning("x-podman in compose file is not a mapping; ignoring it")
            file_settings = {}

        for key, value in file_settings.items():
            known_key = known_keys.get(key)
            if known_key:
                self.x_podman[known_key] = value
            else:
                log.warning(
        ...
        # Process env after the file so PODMAN_COMPOSE_* overrides compose settings.
        excluded = {"PODMAN_COMPOSE_PROVIDER", "PODMAN_COMPOSE_WARNING_LOGS"}
        for env_name, value in environ.items():
            if not env_name.startswith("PODMAN_COMPOSE_") or env_name in excluded:
                continue
            setting_name = env_name.removeprefix("PODMAN_COMPOSE_").lower()
            known_key = known_keys.get(setting_name)
            if not known_key:
                log.warning(
            ...
            if value is None:
                continue
            self.x_podman[known_key] = self._coerce_x_podman_env_value(known_key, str(value))
    ...
    def _parse_compose_file(self) -> None:
        ...
        # Must run before services and default-network behavior are resolved.
