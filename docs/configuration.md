...
: Specifies how long (in seconds) read data should be cached.
    ...
    Allowed values are between `1` (one second) and `31536000` (one year). Defaults to `3600` (one hour). Non-numeric, empty, zero, and negative values are ignored and the default is used.
    ...
    For more details, refer to the Redis eviction policy documentation, and see the `PAPERLESS_READ_CACHE_REDIS_URL` setting to specify a separate Redis broker.
...
: Defines the Redis instance used for the read cache.
    ...
    !!! note
        ...
        If this value is not set, the read cache uses that localhost default rather than `PAPERLESS_REDIS`.
        Set it explicitly (for example to the same URL as [`PAPERLESS_REDIS`](#PAPERLESS_REDIS)) when the broker is not on localhost, or point it at a dedicated Redis instance.
