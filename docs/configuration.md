...
ports.
    ...
    Defaults to an empty list, which allows all ports.
...
: If set to false, webhooks cannot be sent to internal URLs (private,
loopback, or link-local addresses).

    Defaults to false, which blocks internal requests.
