...
: Specifies how long (in seconds) read data should be cached.
    ...
    Allowed values are between `1` (one second) and `31536000` (one year). Defaults to `3600` (one hour). Values that are not positive, or that cannot be parsed as an integer, are ignored and the default is used.
...
: Defines the Redis instance used for the read cache.
    ...
    !!! note
        ...
        If this value is not set, the same Redis instance used for scheduled tasks ([`PAPERLESS_REDIS`](#PAPERLESS_REDIS), default `redis://localhost:6379`) will be used for caching as well.
