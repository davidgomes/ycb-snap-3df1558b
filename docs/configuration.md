...
: Caches database read query results in Redis. This can significantly improve application response times for repeated queries, at the cost of slightly increased memory usage.

    Defaults to `false`. When disabled, queries always hit the database and the caching library is not loaded.
...
: Specifies how long (in seconds) read data should be cached.
    ...
    Allowed values are between `1` (one second) and `31536000` (one year). Defaults to `3600` (one hour). Values that are missing, not integers, or less than or equal to zero are ignored and the default is used. Values above one year are capped at one year.

    !!! warning

        A high TTL increases memory usage over time. Memory may be used until the end of the TTL, even if the cache is invalidated with the `invalidate_cachalot` command.
...
: Defines the Redis instance used for the read cache.
    ...
    !!! note
        ...
        If this value is not set, the same Redis instance used for scheduled tasks (`PAPERLESS_REDIS`, default `redis://localhost:6379`) is used for caching as well.
