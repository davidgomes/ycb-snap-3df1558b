...
-   If no version is specified, Paperless serves the configured default
    API version (currently `7`). Clients that depend on the format of an
    older API version should always request that version explicitly.
...
-   The `extra_data.select_options` of select type custom fields are now
    returned as an array of objects with `id` and `label` fields instead of
    a simple list of strings. Options without an `id` are assigned one when
    the field is created or updated.
-   The value of a select type custom field on a document is now the `id` of
    the selected option instead of its index in `select_options`.
-   Requests using API version 6 or lower keep using option labels and
    indexes for select type custom fields.
-   The default API version, served when a request does not specify one, is
    now `7` instead of `1`.
