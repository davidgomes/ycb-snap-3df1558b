...
variable. When a `PODMAN_COMPOSE_*` variable and the compose file both set the same `x-podman`
key, the environment variable wins. Command-line flags still take precedence over both.
