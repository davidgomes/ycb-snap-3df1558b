...
type ValidationOptionsWithState struct {
	...
	// RollupCostFn is an optional extension, to validate the total rollup costs
	// (L1 cost and operator cost) of a tx
...
func ValidateTransactionWithState(tx *types.Transaction, signer types.Signer, opts *ValidationOptionsWithState) error {
	...
	if overflow {
		return fmt.Errorf("%w: total tx cost overflows 256 bits", core.ErrInsufficientFunds)
	}
	cost := cost256.ToBig()
