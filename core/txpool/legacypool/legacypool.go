...
type LegacyPool struct {
	...
	rollupCostFn txpool.RollupCostFunc // Total rollup cost (L1 + operator) at the current head, optional field, may be nil.
}
...
// RollupCostFunc returns the pool's current rollup cost function, which may be nil.
		...
		ExistingCost: func(addr common.Address, nonce uint64) *big.Int {
			if list := pool.pending[addr]; list != nil {
				if tx := list.txs.Get(nonce); tx != nil {
					// Use the total cost the tx was added with, as accounted in list.totalcost.
...
// resetRollupCostFn sets the pool's rollup cost function to compute the total rollup
// cost (L1 data availability and operator costs) against the given head state and time.
