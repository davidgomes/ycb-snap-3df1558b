...
func (l *list) Add(tx *types.Transaction, priceBump uint64) (bool, *types.Transaction) {
	...
	// Add new tx cost to totalcost. This includes value, gas, L1 rollup cost, and
	// operator fees so later balance checks see the full amount the sender must cover.
...
func (l *list) subTotalCost(txs []*types.Transaction) {
	for _, tx := range txs {
		...
		// The cached value is the cost at insertion time, including the operator fee and L1 cost
		// that applied then, so a later fee-parameter change cannot desync the running total.
		cost := l.txCosts[h]
