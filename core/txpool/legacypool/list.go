...
func (l *list) Add(tx *types.Transaction, priceBump uint64) (bool, *types.Transaction) {
	...
	// Add new tx cost to totalcost, including L1 data availability and operator fees.
	cost, overflow := txpool.TotalTxCost(tx, l.rollupCostFn())
	if overflow {
		return false, nil
	}
	l.txCosts[tx.Hash()] = cost // OP-Stack addition: cache because rollup cost can change per block
