...
// NewTotalRollupCostFunc returns a TotalRollupCostFunc that computes the total rollup cost, consisting
// of both the data availability cost and the operator cost, or nil if this is not an op-stack chain.
// The operator cost is only charged once Isthmus is active.
func NewTotalRollupCostFunc(config *params.ChainConfig, statedb StateGetter) TotalRollupCostFunc {
	...
	return func(tx RollupTransaction, blockTime uint64) *uint256.Int {
		// caching of the fee parameters happens inside the individual cost functions
		...
		if operatorCost != nil {
			if _, overflow := totalCost.AddOverflow(totalCost, operatorCost); overflow {
...
// ExtractEcotoneFeeParams extracts the L1 base fee scalar and L1 blob base fee scalar from the
// L1FeeScalarsSlot storage slot of the L1Block contract.
...
// ExtractOperatorFeeParams extracts the operator fee scalar and operator fee constant from the
// OperatorFeeParamsSlot storage slot of the L1Block contract.
